...
private enum IslandTheme {
    ...
    static let setupBg = Color.black.opacity(0.82)
...
struct DynamicIslandView: View {
    ...
    /// Callback to notify the window about interaction mode changes
    ...
    private var currentSize: CGSize {
        ...
        case .setup: return CGSize(width: 300, height: 195)
    ...
    private var setupContent: some View {
        VStack(spacing: 12) {
            ...
            IslandTextField(text: $taskInput, placeholder: "What are you working on?")
            ...
            HStack(spacing: 6) {
                ...
                IslandCustomDuration(text: $customMinutes, onChanged: {
                    if !customMinutes.isEmpty { selectedMinutes = nil }
                })
            }
        ...
        TextField("", text: $text, prompt: Text(placeholder).foregroundStyle(.white.opacity(0.3)))
            ...
            .background(
                RoundedRectangle(cornerRadius: 10)
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 10)
                            .strokeBorder(.white.opacity(isFocused ? 0.2 : 0.08), lineWidth: 1)
                    )
            )
            .focused($isFocused)
    ...
    var body: some View {
        Button(action: action) {
            Text(label)
                ...
                .background(
                    Capsule()
                        .fill(isSelected ? IslandTheme.teal : .white.opacity(0.1))
                )
        }
        .buttonStyle(.plain)
    }
...
private struct IslandCustomDuration: View {
    ...
    var body: some View {
        HStack(spacing: 4) {
            TextField("", text: $text, prompt: Text("—").foregroundStyle(.white.opacity(0.3)))
                .textFieldStyle(.plain)
    ...
    var body: some View {
        Button(action: action) {
            Text("Start Focus")
                ...
                .background(
                    Capsule()
                        .fill(disabled ? .white.opacity(0.06) : IslandTheme.teal)
                )
        }
        .buttonStyle(.plain)
        .disabled(disabled)
